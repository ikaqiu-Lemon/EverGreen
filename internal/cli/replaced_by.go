...
//
// 语义方向**只有一个**：在**被替代的**知识卡或观点（`--target`）上写 `replaced_by`，
// 指向新的知识卡或观点（`--to`）。宿主与指向端都是跨类型端点（k- / o-）：迁移会把一条判断
// 从知识卡改成观点，此时原 k- 卡逻辑删除并用 replaced_by 指向新 o- 观点（Schema v2 §9.2 / T-009）。
// 被指向的替代端点**一个字节都不动**——反向写入会让「谁替代了谁」出现两份可漂移的记录，
// 反向查询归 S4，本阶段既不写反向指针、也不建索引。
//
// 与另两条状态命令共用 buildStateOpPlan → runPlan：E10（宿主或指向端已逻辑删除 → 退 2 零写入）
...
// runSetReplacedBy 实现 eg replaced-by --target <k|o-id> --to <k|o-id> --reason <text>。
func (r *Root) runSetReplacedBy(inv *Invocation) (*Result, error) {
	...
	if res != nil {
		res.Summary = append([]string{fmt.Sprintf(
			"replaced-by：在 %s 上写 replaced_by → %s（单向；被指向端点字节不变）",
