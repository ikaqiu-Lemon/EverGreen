...
// check 十三值封闭枚举、severity 恰两值、check ↔ 诊断码双向单射（对账合同 §3；A-62 起含 W29）。
//
...
//     多写第 14 行或漏写一行都编译不过；severityTable 同型（长度恰 SeverityCount）。
//   - **运行期**：ParseCheck / ParseSeverity / ParseCode 拒收表外取值。
//   - **表驱动用例**：check_test.go 的 TestCheckEnumExactlyThirteenValues /
//     TestCheckDiagnosticBijection / TestCheckEnumRejectsFourteenthValue 三组。

import "fmt"

// check 的十三值封闭枚举（值与顺序 = 合同 §3 表格第 1 ~ 12 行 + A-62 追加第 13 行，逐字）。
const (
	...
	CheckOpinionUnsupportedValidated = "opinion_unsupported_validated" // 13 · R3（A-62 新增）
)
...
const (
	...
	// CodeW29 是 A-62 新增的 opinion_unsupported_validated（R3 第五项）。
	// W21–W28 已各有归属 / 属其它域，W29 是本次唯一解冻的新 warning 码。
	CodeW29 = "W29" // opinion_unsupported_validated（A-62）
)

// 封闭计数（判据与用例逐字消费；改动这三个数字必须同步合同 §3 与 M-004 判据 3）。
const (
	// CheckCount 是 check 枚举的封闭基数：恰 13（A-62 起含 opinion_unsupported_validated）。
	CheckCount = 13
	// SeverityCount 是 severity 的封闭基数：恰 2。
	SeverityCount = 2
	// CodeCount 是本包新增诊断码的封闭基数：恰 13（E11–E14 四个 + W13–W20 八个 + W29 一个）。
...
var checkTable = [CheckCount]CheckSpec{
	...
	{CheckOpinionUnsupportedValidated, SeverityWarning, CodeW29, "R3"},
}
...
// 十三个 check 里十二个的 targets 是**集合**（一批可定位标识，次序不承载语义）。
...
// Specs 返回十三行封闭表的副本（顺序 = 合同 §3 表格行序；调用方改副本不影响真源）。
...
// IsKnownCode 报告取值是否落在本包十三值诊断码集合内。
...
// ParseCode 把外部字符串收敛到本包诊断码集合（W 段为 W13–W20 加 W29，越号一律报错）。
