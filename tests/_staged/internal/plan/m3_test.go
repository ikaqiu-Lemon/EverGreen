...
func TestDiagnosticCodes_Closed(t *testing.T) {
	...
	// S3 检查器包的 check 枚举（M4 十二值起，A-62 修订后为十三值），那批编号在该包内是**合同要求存在**的事实。因此把判据
	...
	//   - S3 检查器包内部：只许 M4 已发放的 E11–E14 / W13–W20，外加 A-62 正式修订发放的
	//     R3·W29（opinion_unsupported_validated），其余 E15+ / W21–W28 / W30+ / I2+
	//     一律零命中（M5–M6 的号段照旧留白）；
	...
	m4Reserved := regexp.MustCompile(`"(E1[5-9]|E[2-9][0-9]|W2[1-8]|W[3-9][0-9]|I[2-9])"`)
